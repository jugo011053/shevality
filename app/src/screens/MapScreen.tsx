...
const T = {
  ...
  city: { de: 'Frankfurt', en: 'Frankfurt' },
  list: { de: 'Liste', en: 'List' },
  nativeTitle: { de: 'Karte', en: 'Map' },
  nativeText: {
    de: 'Die interaktive Karte gibt es in der Web-Version – öffne Shevality im Browser oder über das zum Homescreen hinzugefügte App-Symbol. Tippe auf „Liste" für die Übersicht.',
    en: 'The interactive map is available in the web version – open Shevality in the browser or via the app icon added to your home screen. Tap “List” for the overview.',
  },
  toList: { de: 'Zur Liste', en: 'Go to list' },
};
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }} edges={['top']}>
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 22, paddingTop: 10, paddingBottom: 10 }}>
        <View>
          <Text style={{ fontFamily: fonts.young, fontSize: 26, color: colors.ink, letterSpacing: -0.3 }}>{t(T.title)}</Text>
          <Text style={{ fontFamily: fonts.instrumentItalic, fontSize: 13, color: colors.pink, marginTop: 2 }}>{t(T.city)}</Text>
        </View>
        <Pressable
        ...
        >
          ...
          <Text style={{ fontFamily: fonts.hanken600, fontSize: 12.5, color: colors.ink }}>{t(T.list)}</Text>
        </Pressable>
      </View>
      ...
      <View style={{ flex: 1, overflow: 'hidden' }}>
        {Platform.OS === 'web' ? (
          ...
          <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 40, gap: 10 }}>
            ...
            <Text style={{ fontFamily: fonts.young, fontSize: 20, color: colors.ink, textAlign: 'center' }}>{t(T.nativeTitle)}</Text>
            ...
            <Pressable onPress={() => navigation.navigate('EntdeckenList')} style={{ marginTop: 8, paddingVertical: 11, paddingHorizontal: 20, borderRadius: 24, backgroundColor: colors.purple }}>
              <Text style={{ fontFamily: fonts.hanken600, fontSize: 13, color: colors.white }}>{t(T.toList)}</Text>
            </Pressable>
          </View>
        )}
      </View>
